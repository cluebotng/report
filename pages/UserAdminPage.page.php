        ...
        public function __construct()
        {
            ...
            if (isset($_REQUEST[ 'action' ])) {
                ...
                switch ($_REQUEST[ 'action' ]) {
                    case 'delete':
                        mysqli_query('UPDATE `comments` SET `userid` = -1 WHERE `userid` = \'' . mysqli_real_escape_string($_REQUEST[ 'uid' ]) . '\'');
                        mysqli_query('UPDATE `reports` SET `reporterid` = -1 WHERE `reporterid` = \'' . mysqli_real_escape_string($_REQUEST[ 'uid' ]) . '\'');
                        mysqli_query('DELETE FROM `users` WHERE `userid` = \'' . mysqli_real_escape_string($_REQUEST[ 'uid' ]) . '\'');
                        break;
                    case 'superadmin':
                        $set = '`superadmin` = 1';
                        break;
                ...
                if ($set !== null) {
                    mysqli_query('UPDATE `users` SET ' . $set . ' WHERE `userid` = \'' . mysqli_real_escape_string($_REQUEST[ 'uid' ]) . '\'');
                }
            ...
            $result = mysqli_query('SELECT `userid`, `username`, `email`, `admin`, `superadmin` FROM `users`');
            $this->users = array();
