    ...
    class OptionsPage extends Page
    {
                public function __construct()
                {
                    if (isset($_POST[ 'submit' ])) {
                        ...
                        $query = "UPDATE `users` SET `next_on_review` = '" . mysqli_real_escape_string($next_on_review) . "'";
                        $_SESSION[ 'next_on_review' ] = ($next_on_review) ? true : false;

                        if (trim($_POST[ 'email' ]) != "") {
                            $query .= ", `email` = '" . mysqli_real_escape_string($_POST[ 'email' ]) . "'";
                            $_SESSION[ 'email' ] = mysqli_real_escape_string($_POST[ 'email' ]);
                        }

                        if (trim($_POST[ 'password' ]) != "") {
                            $query .= ", `password` = PASSWORD('" . mysqli_real_escape_string($_POST[ 'password' ]) . "')";
                        }
